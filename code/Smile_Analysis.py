...
def smile_presence_analysis():
    tutor = {}
    tutee = {}
    both = {}
    for index,row in df.iterrows():
        if row["Smile_Tutor"] == "x" and row["Smile_Tutee"] == "x":
            if row["Rapport Rating"] in both:
                both[row["Rapport Rating"]] += 1
            else:
                both[row["Rapport Rating"]] = 1
        ...
        elif  row["Smile_Tutor"] == "x":
            if row["Rapport Rating"] in tutor:
                tutor[row["Rapport Rating"]] += 1
            else:
                tutor[row["Rapport Rating"]] = 1
        
        elif  row["Smile_Tutee"] == "x":
            if row["Rapport Rating"] in tutee:
                tutee[row["Rapport Rating"]] += 1
            else:
                tutee[row["Rapport Rating"]] = 1
    ...
    for i in range(1,8):
        key = float("%d.0" % i)
        print(key, both[key], tutor[key], tutee[key])
...
print("\nDoing Smile Presence Analysis...\n")
smile_presence_analysis()
print("\nSmile Presence Analysis done.\n")
